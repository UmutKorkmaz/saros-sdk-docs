...
export class DLMMCalculator {
   ...
   */
  async calculateRebalanceStrategy(params: {
    ...
    const currentILResult = await this.calculateConcentratedIL(position);
    ...
    const newILResult = await this.calculateConcentratedIL(newPosition);
