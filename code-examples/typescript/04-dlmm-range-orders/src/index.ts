...
async function runDemoScenarios(connection: Connection, wallet: Keypair) {
  ...
  await demonstrateLimitBuy(manager);
  ...
  logger.info('\n=== Scenario 5: Stop Loss Protection ===');
  await demonstrateStopLoss(manager);
}

/**
 * Demonstrate limit buy order
 */
async function demonstrateLimitBuy(manager: RangeOrderManager) {
...
async function demonstrateBuyLadder(manager: RangeOrderManager) {
  ...
  ladder.orders.forEach((order, i) => {
    logger.info(`  ${i + 1}. $${order.price.toFixed(2)} - ${order.amount} USDC`);
  });
...
async function demonstrateTakeProfit(manager: RangeOrderManager) {
  ...
  const takeProfits = await manager.createTakeProfitLevels({
    ...
    levels: [
      { price: position.entryPrice * 1.05, percentage: 25 }, // 5% profit, sell 25%
      { price: position.entryPrice * 1.10, percentage: 25 }, // 10% profit, sell 25%
      { price: position.entryPrice * 1.15, percentage: 25 }, // 15% profit, sell 25%
      { price: position.entryPrice * 1.20, percentage: 25 }, // 20% profit, sell 25%
    ]
  });
 ...
 */
async function demonstrateStopLoss(manager: RangeOrderManager) {
  const currentPrice = await manager.getCurrentPrice(SOL_USDC_POOL);
  logger.info(`Current SOL price: $${currentPrice.toFixed(2)}`);
  ...
  const stopLoss = await manager.createStopLoss({
    ...
    amountSOL: 50,
    orderType: 'range', // 'range' or 'market'
    slippage: 0.5 // For market orders
  });
  
  logger.info(`Stop loss created:`);
  logger.info(`- Trigger: $${stopLoss.triggerPrice.toFixed(2)}`);
  logger.info(`- Amount: ${stopLoss.amount} SOL`);
  logger.info(`- Type: ${stopLoss.orderType}`);
  logger.info(`- Max loss: ${stopLoss.maxLoss}%`);
  
  // Monitor for trigger
  const monitor = new RangeOrderMonitor(connection);
  monitor.watchStopLoss(stopLoss.id, async (triggered) => {
    if (triggered) {
      logger.warn('Stop loss triggered! Executing sell order...');
      await manager.executeStopLoss(stopLoss.id);
    }
  });
}
